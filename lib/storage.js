...
fileUtils.init = (options) => {
    ...
    return {
        ...
        remove: fileUtils.removeData(options),
    }
};
...
dbUtils.init = (options) => {
    ...
    return {
        ...
        remove: dbUtils.removeData(options, model),
    };
};
...
dbUtils.removeData = (options, model = mongoose.Model) => {
    return (token, key = '', callback = (err, data) => { }) => {
        if (typeof key === 'function') {
            callback = key;
            key = null;
